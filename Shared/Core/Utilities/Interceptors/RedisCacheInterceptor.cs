...
{
    public class RedisCacheInterceptor: AbstractInterceptor
	{
		[FromContainer]
		...
		{
			RedisCacheAttribute attribute = context.GetAttribute<RedisCacheAttribute>();
		...
		private async Task SetCache(string cacheKey, object cacheValue, int expiration)
		{
			...
			await Database.StringSetAsync(cacheKey, jsonValue, TimeSpan.FromMinutes(expiration));
		}
