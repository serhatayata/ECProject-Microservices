...
namespace Core.Utilities.Security.Jwt
{
    public class JwtHelper : ITokenHelper
    {
        ...
        #region CreateToken
        public AccessToken CreateToken(IdentityUser user, List<Claim> operationClaims, int companyId = 0)
        {
        ...
        public JwtSecurityToken CreateJwtSecurityToken(TokenOptions tokenOptions, IdentityUser user,
            SigningCredentials signingCredentials, List<Claim> operationClaims, int companyId = 0)
        {
            if (companyId == 0)
            {
        ...
        public RefreshToken CreateRefreshToken(IdentityUser user, List<Claim> operationClaims = null, int companyId=0)
        {
            ...
            {
                var jwt = CreateJwtSecurityRefreshToken(_tokenOptions, user, signingCredentials, operationClaims, companyId);
                var jwtSecurityTokenHandler = new JwtSecurityTokenHandler();
                var token = jwtSecurityTokenHandler.WriteToken(jwt);
                return new RefreshToken
                {
        ...
        #endregion
        #region SetClaims
        private IEnumerable<Claim> setClaims(IdentityUser user, List<Claim> operationClaims, int companyId = 0)
        {
            var claims = new List<Claim>();
            claims.AddNameIdentifier(user.Id);
            if (companyId!=0)
            {
                claims.AddUserData(companyId.ToString());
            }
